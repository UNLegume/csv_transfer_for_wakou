...
作成された次のファイルに、Shopify接続情報、ログイン情報、発送元、ヤマト・佐川の契約情報を設定します。
...
| 依頼主 | 依頼主コード、氏名、電話番号、郵便番号、住所 |
| 品名 | 固定設定値。標準は「ネットショップ購入品」 |
| 代引き | 代金引換額、消費税額の設定値 |
| 請求情報 | 請求先分類コード、運賃管理番号 |
| 空欄項目 | 配達指定、時間指定、会社名、カナ、荷扱い、営業所止めなど |
...
| 届け先 | 氏名、電話番号、郵便番号、住所 |
| 出荷元・依頼主 | 氏名、電話番号、郵便番号、住所 |
| 指示文言 | 佐川設定の`option1`〜`option6` |
| 支払い設定 | 郵便種別、元／着払い種別、代引き種別 |
| 契約情報 | 佐川請求コード |
| 空欄項目 | 時間指定、シールコード、営業所止め、備考、品名など |
